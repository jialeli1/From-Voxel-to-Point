...
def build_optimizer(model, optim_cfg, logger, fixed=False):
    if optim_cfg.OPTIMIZER == 'adam':
        ...
        # fix rpn: do this like PointRCNN since we use costomized optimizer.step
        # ###################临时添加##########################
        # 但是这里仍然只是对conv fc层固定住了，还有BN层需要使用.eval()
        if fixed:
            for param in model.vfe.parameters():
                logger.info('==> set no grad for vfe.')
                param.requires_grad = False
            for param in model.backbone_3d.parameters():
                logger.info('==> set no grad for backbone_3d.')
                param.requires_grad = False
            for param in model.backbone_2d.parameters():
                logger.info('==> set no grad for backbone_2d.')
                param.requires_grad = False    
            for param in model.dense_head.parameters():
                logger.info('==> set no grad for dense_head.')
                param.requires_grad = False   
        # ###################临时添加##########################
    else:
