...
class RotationNormalizeCoder(object):
    def __init__(self, **kwargs):
        super().__init__()
        self.code_size = 1

    def encode_torch(self, rots):
        rot_encodings = ( rots % (2 * np.pi) ) / (2 * np.pi)
        return rot_encodings

    def decode_torch(self, rot_encodings):
        rots = rot_encodings * (2 * np.pi)

        # TODO: 这里需要这样处理吗？
        rots[rots > np.pi] -= 2 * np.pi

        return rots

class RotationTanCoder(object):
    def __init__(self, **kwargs):
        super().__init__()
        self.code_size = 2

    def encode_torch(self, rots):
        # print('==> rots.shape: ', rots.shape) # [..., 1]
        rot_cos_encodings = torch.cos(rots)
        rot_sin_encodings = torch.sin(rots)

        rot_encodings = torch.cat([rot_cos_encodings, rot_sin_encodings], dim=-1)
        # print('==> rot_encodings.shape: ', rot_encodings.shape) # [..., 2]
        assert rot_encodings.shape[-1] == self.code_size
    ...
    def decode_torch(self, rot_encodings):
        ...
        cost, sint = torch.split(rot_encodings, 1, dim=-1)
        # print('==> cost.shape: ', cost.shape) # [..., 1]
        # print('==> sint.shape: ', sint.shape) # [..., 1]

        rots = torch.atan2(sint, cost)
