...
class OpenPCDetWaymoDetectionMetricsEstimator(tf.test.TestCase):
    ...
    def mask_by_distance_lr(self, distance_thresh, boxes_3d, *args):
        distance_l = 0.
        distance_r = 30.

        distance = np.linalg.norm(boxes_3d[:, 0:2], axis=1)

        mask_o = distance < distance_thresh + 0.5
        mask_l = distance >= distance_l
        mask_r = distance < distance_r

        mask = mask_o * mask_l * mask_r
