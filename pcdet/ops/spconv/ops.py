...
def indice_group(features,
                ...
                subm=False):
    if features.dtype == torch.float32:
        return sparse_conv_ext.indice_group_fp32(features,
                                                indice_pairs, indice_pair_num,
                                                num_activate_out, int(inverse),
                                                int(subm))
    elif features.dtype == torch.half:
...
def indice_group_backward(features,
                         out_bp,
                         indice_pairs,
                         indice_pair_num,
                         inverse=False,
                         subm=False):
    if features.dtype == torch.float32:
        return sparse_conv_ext.indice_group_backward_fp32(
            features, 
            out_bp, 
            indice_pairs, 
            indice_pair_num,
