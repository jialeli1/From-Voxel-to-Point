...
def class_agnostic_nms_withfgscore(box_fgscores, box_scores, box_preds, nms_config, score_thresh=None):
    """
    box_fgscores: filter out the negative results.
    box_scores: loc score for NMS
    """
    src_box_scores = box_scores
    if score_thresh is not None:
        scores_mask = (box_fgscores >= score_thresh)
        box_scores = box_scores[scores_mask]
        box_preds = box_preds[scores_mask]
