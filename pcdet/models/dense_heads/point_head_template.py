...
class PointHeadTemplate(nn.Module):
    ...
    def assign_stack_targets(self, points, gt_boxes, extend_gt_boxes=None,
                             ret_box_labels=False, ret_part_labels=False, ret_vote_labels=False,
        ...
        point_vote_labels = gt_boxes.new_zeros((points.shape[0], 3)) if ret_vote_labels else None
        ...
        for k in range(batch_size):
            ...
            if ret_vote_labels and gt_box_of_fg_points.shape[0] > 0: # 条件和box_labels一致
                point_vote_labels_single = point_vote_labels.new_zeros((bs_mask.sum(), 3))
                transformed_points = gt_box_of_fg_points[:, 0:3] - points_single[fg_flag]
                
                point_vote_labels_single[fg_flag] = transformed_points
                point_vote_labels[bs_mask] = point_vote_labels_single

        targets_dict = {
            'point_cls_labels': point_cls_labels,
            'point_box_labels': point_box_labels,
            'point_part_labels': point_part_labels,
            'point_vote_labels': point_vote_labels
        }
        return targets_dict
    ...
    def get_double_cls_layer_loss(self, tag='conv_', tb_dict=None):
        """
        conv_point_cls_preds: torchTensor
        conv_point_cls_labels: torchTensor
        trans_point_cls_preds: torchTensor
        trans_point_cls_labels: torchTensor
        """
        point_cls_labels = self.forward_ret_dict[tag + 'point_cls_labels'].view(-1)
        point_cls_preds = self.forward_ret_dict[tag + 'point_cls_preds'].view(-1, self.num_class)
        ...
        if tb_dict is None:
            tb_dict = {}
        tb_dict.update({
            tag + 'point_loss_cls': point_loss_cls.item(),
            tag + 'point_pos_num': pos_normalizer.item()
        })
        return point_loss_cls, tb_dict
    ...
    def get_vote_layer_loss(self, tb_dict=None):
        ...
        # print(self.forward_ret_dict.keys())
        pos_mask = self.forward_ret_dict['point_cls_labels'] > 0
        point_vote_labels = self.forward_ret_dict['point_vote_labels']
        point_vote_preds = self.forward_ret_dict['point_vote_preds']
        ...
        point_loss_vote_src = self.reg_loss_func(
            point_vote_preds[None, ...], point_vote_labels[None, ...], weights=reg_weights[None, ...]
        )
        point_loss_vote = point_loss_vote_src.sum()

        loss_weights_dict = self.model_cfg.LOSS_CONFIG.LOSS_WEIGHTS
        point_loss_vote = point_loss_vote * loss_weights_dict['point_vote_weight']
        # loss_weights_vote = 1.0
        # point_loss_vote = point_loss_vote * loss_weights_vote
        if tb_dict is None:
            tb_dict = {}
        tb_dict.update({'point_loss_vote': point_loss_vote.item()})
        return point_loss_vote, tb_dict
