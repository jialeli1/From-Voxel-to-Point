...
class ProposalTargetLayer(nn.Module):
    ...
    def subsample_rois(self, max_overlaps):
        ...
        fg_inds = torch.nonzero( (max_overlaps >= fg_thresh), as_tuple=False).view(-1) # torch 1.5+

        # easy_bg_inds = ((max_overlaps < self.roi_sampler_cfg.CLS_BG_THRESH_LO)).nonzero().view(-1)
        easy_bg_inds = torch.nonzero( (max_overlaps < self.roi_sampler_cfg.CLS_BG_THRESH_LO), as_tuple=False ).view(-1) # torch 1.5+
        ...
        hard_bg_inds = torch.nonzero( ((max_overlaps < self.roi_sampler_cfg.REG_FG_THRESH) &
                (max_overlaps >= self.roi_sampler_cfg.CLS_BG_THRESH_LO)), as_tuple=False).view(-1) # torch 1.5+

        fg_num_rois = fg_inds.numel()
        bg_num_rois = hard_bg_inds.numel() + easy_bg_inds.numel()

        # print('==> expected fg_rois_per_image: ', fg_rois_per_image) # 64
        # print('==> actual fg_num_rois: ', fg_num_rois) # 180 ~ 250
        if fg_num_rois > 0 and bg_num_rois > 0:
            ...
            # print('==> final fg_rois_per_this_image: ', fg_rois_per_this_image) # 64
